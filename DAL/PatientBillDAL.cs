//using ALHMobileAppAPI.AdminService;
//using ALHMobileAppAPI.ARService;
//using ALHMobileAppAPI.BillingCalculationService;
//using ALHMobileAppAPI.BillingFacadeService;
//using ALHMobileAppAPI.CommonSearch;
//using ALHMobileAppAPI.ContractMgmtService;
//using ALHMobileAppAPI.FrontOfficeService;
//using ALHMobileAppAPI.IPBillingService;
using RCMAPI.Models;
...
namespace RCMAPI.DAL
{
    public class PatientBillDAL
    {
        ...
        static string MODULE_NAME = "PatientBill";
        ...
        string hdnIsDefaultLOA = "false"; string strERExceedMSG = string.Empty;
        ...
        PatientBillInfoList objPatientList = new PatientBillInfoList();
        ...
        public PatientBillInfoList PatientBillSummary(PatientBillList PatientBillList)
        {
            ...
            try
            {
                ...
                //Skipped PatientHighrisk -- Need to implement
                ...
                if (DTTemLoadOrders != null)
                {
                    ...
                    Base obj = PaymentCheck(PatientBillList, PatientUHID, hdnIsfamilyLOA);
            ...
            catch (Exception ex)
            {
                ...
                objPatientList.Status = ProcessStatus.Fail.ToString();
                objPatientList.Message = ex.Message;
                HIS.TOOLS.Logger.ErrorLog.ErrorRoutine(ex, MODULE_NAME, "Error in GettingPatientList", "");
            }
            finally
            {
                objDataHelper = null;
        ...
        public DataSet FetchActivePackage(int intPackageId, int intUserId, string strTableId)
        {
            objFOClient = new FrontOfficeServiceContractClient();
            DataSet dsGetActivePackage = new DataSet();
            try
            {
                dsGetActivePackage = objFOClient.FetchActivePackage(intPackageId, intUserId, strTableId);
            }
            finally
            {
                objFOClient.Close();
            }
            return dsGetActivePackage;
        }
        ...
        public DataSet FetchScheduledConsultations(int intPatientID, int intNoOfDays, int intHospitalsId)
        {
            objFOClient = new FrontOfficeServiceContractClient();
            try
            {
                return objFOClient.FetchScheduledConsultations(intPatientID, intNoOfDays, intHospitalsId);
            }
            finally
            {
                objFOClient.Close();
        ...
        public DataTable FetchADVProcedureDetailsM(string strProcedureName, string strType, string strFilter, int intUserid, int intWorkstationid, int intError)
        {
            try
            {
                objFOClient = new FrontOfficeServiceContractClient();
                return objFOClient.FetchADVProcedureDetails(strProcedureName, strType, strFilter, intUserid, intWorkstationid, intError).Tables[0];
            }
        ...
        public DataSet FetchFetchPatientMonitoringDetails(int intPatientID, int intType, string strConsFilter, int intUserID, int intWorkstationID, int intError)
        {
            objFOClient = new FrontOfficeServiceContractClient();
            try
            {
                return objFOClient.FetchFetchPatientMonitoringDetails(intPatientID, intType, strConsFilter, intUserID, intWorkstationID, intError);
            }
        ...
        public DataSet FetchMISProcedureDetails(string ProcedureName, string sSelect, string sFilter, string sOrder, int UserId, int WstationId, int intError, bool IsMIS)
        {
            try
            {
                objFOClient = new FrontOfficeServiceContractClient();
                return objFOClient.FetchMISProcedureDetails(ProcedureName, sSelect, sFilter, sOrder, UserId, WstationId, intError, IsMIS);
            }
            finally
            {
                objFOClient.Close();
        ...
        private DataSet GetCompanyValidations()
        {
            DataSet dsSearchResults = null;
            try
            {
                CommonSearchServiceContractClient searchService = new CommonSearchServiceContractClient();
                dsSearchResults = searchService.GetSearchResultsHospitalWise("OPBillPayerSearch", 1, 10, "Blocked = 0 and CompanyType='C' AND  CompanyID ='" + strCompanyID + "'", Convert.ToInt32(strDefaultUserId), Convert.ToInt32(strDefWorkstationId), -4, 0, string.Empty, "3", Convert.ToInt32(strDefaultHospitalId));

                if (dsSearchResults != null)
                {
                    // objSearchResponse.SearchResults = dsSearchResults;
            ...
            {
                HIS.TOOLS.Logger.ErrorLog.ErrorRoutine(ex, MODULE_NAME, "Error in GetCompanyValidations", "");
                return null;
            }
        }
        ...
        public PatientBillInfoList PaymentCheck(PatientBillList PatientBillList, string PatientUHID, string hdnIsfamilyLOA)
        {
            try
            {
                string TypeMapi, FilterMAPI, OrderMAPI = string.Empty;
        ...
        public DataSet GetCompanyDeductables(int intCompanyid, string strCompanyType, string strTableid, int intGradeid, int intUserId, int intWorkstationid)
        {
            PBillingServiceContractClient objIPBillingClient = new PBillingServiceContractClient();
            try
            {

                return objIPBillingClient.GetCompanyDeductables(intCompanyid, strCompanyType, strTableid, intGradeid, intUserId, intWorkstationid);
            }

            finally
            {
                objIPBillingClient.Close();
            }
        }
        ...
        public void GetDoctorAvailability()
        {
            try
            {
                DateTime strdate = Convert.ToDateTime(DateTime.Now.ToString(strDataFormat));
                DataSet dsDoctoravilability = ValidateResourceSlotsAvail(Convert.ToInt32(hdnDocID), strdate, 1, 0, 0, null);
                if (dsDoctoravilability != null)
                    if (dsDoctoravilability.Tables[0].Rows.Count > 0)
                    {
                        if (dsDoctoravilability.Tables[0].Rows[0][0].ToString() == "1")
                        {
                            if (System.Configuration.ConfigurationSettings.AppSettings["ResourceAvailability"].ToString().ToLower() == "yes")
                            {
                                // ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','Doctor is not Available <BR> Do you want to continue?" + "','YESNO','Information');", true);
                                DocAvail = "YES";
                            }
                            else
                                return;
        ...
        public DataSet ValidateResourceSlotsAvail(int DoctorID, DateTime Fromdate, int HospitalID, int FeatureId, int FunctionId, string CallContext)
        {
            DataSet dsDoctor = new DataSet();
            FrontOfficeServiceContractClient objResource = new FrontOfficeServiceContractClient();
            try
            {
                dsDoctor = objResource.ValidateResourceSlotsAvail(DoctorID, Fromdate, HospitalID, FeatureId, FunctionId, CallContext);
                return dsDoctor;
            }
            finally
            {
        ...
        protected void imgItemprocedure_Click(PatientBillList PatientBillList)
        {
            int intSeq = 0;
            try
            {
                DataTable DTTemNew = null; DataTable dtItemPrices = null; DataTable DtOPServices = null; DataRow[] dtRow = null;
                strValidatePrfileAndPHCItems = ConfigurationSettings.AppSettings["OpBillingValidationofProfileAndPHC"].ToString();
                DataTable searchData = null; string[] StrOrdTyp = null;
                DataTable dtSearchData = null;
                if (searchData != null)
                {
                    dtSearchData = (DataTable)searchData.Copy();
                    dtSearchData.AcceptChanges();
                }
                if (dtSearchData != null)
                {
                    searchData = dtSearchData;
                    searchData = null;
                    if (searchData.Columns.Contains("OrderType"))
                    {
                        OrderTypeM = searchData.Rows[0]["OrderType"].ToString();
                    }
                    else
                    {
                        OrderTypeM = string.Empty;
                    }
                    string strMultiSearchItems = string.Empty;
                    DTTemNew = CreateDTTemp();
                    DTTemNew.Columns.Add("SlNo", typeof(int)).AutoIncrement = true;
                    DTTemNew.Columns["SlNo"].AutoIncrementSeed = 1;

                    DTTemNew.AcceptChanges();
                    DataRow drNewRow = null;
                    for (int j = 0; j < searchData.Rows.Count; j++)
                    {
                        drNewRow = DTTemNew.NewRow();

                        string strsubrecord = "";

                        if (ddlservices == "5" || ddlservices == "3")
                        {
                            IsAppliedProfessionalCharge = searchData.Rows[j]["IsAppProfChar"] is DBNull ? false : Convert.ToBoolean(searchData.Rows[j]["IsAppProfChar"]);
                        }
                        if (ddlservices == "2")
                        {
                            int isApliedProf = searchData.Rows[j]["IsProfChargesApplicable"] is DBNull ? 0 : Convert.ToInt32(searchData.Rows[j]["IsProfChargesApplicable"]);
                            if (isApliedProf == 1)
                                IsAppliedProfessionalCharge = true;
                        }

                        SetSearchValuesToDataRow(j, searchData, drNewRow);

                        if (searchData.Columns.Contains("EmpID"))
                        {
                            string doctorid = strsubrecord;
                            string strDocName = string.Empty;
                            Doctorid = searchData.Rows[j]["EmpId"].ToString();
                            hdnDocID = Doctorid.ToString();
                            GetDoctorAvailability();
                            #region consultation for cash & credit hdnvalues
                        }
                        if (searchData.Columns.Contains("SpecialiseID"))
                        {
                            if (searchData.Rows[j]["SpecialiseID"] == DBNull.Value || searchData.Rows[j]["SpecialiseID"].ToString() == string.Empty || searchData.Rows[j]["SpecialiseID"].ToString() == "")
                            {
                                specilaize = "null";
                            }
                            else
                            {
                                specilaize = searchData.Rows[j]["SpecialiseID"].ToString();
                                if (specilaize != null && specilaize.ToString() != string.Empty && searchData.Rows[j]["ServiceID"].ToString() == "2")
                                {
                                    hdnDocSpecialiseId = specilaize.ToString();
                                }
                            }
                            #endregion
                        }
                        if (!string.IsNullOrEmpty(drNewRow["ProcedureID"].ToString()))
                        {
                            int intTariffid = Convert.ToInt32(hdnTariffID);
                            string strBillBedType = "-1";
                            if (hdnPrice != "Investigation")
                            {
                                switch (hdnPrice)
                                {
                                    case "Employee Service":
                                        {
                                            if (drNewRow["SampleId"] is DBNull)
                                            {
                                                //ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','No Consultation Types created','OK','Error');", true);
                                                return;
                                            }
                                            else
                                            {
                                                if (drNewRow["specialiseId"] != DBNull.Value)
                                                {
                                                    hdnProcedureID = drNewRow["ProcedureID"].ToString();
                                                    if (hdnrblbilltypeCredit == true)
                                                    {
                                                        DataSet dsCompanyforLOA = null;
                                                        if (hdnrblbilltypeCredit == true && !string.IsNullOrEmpty(strCompanyID))
                                                        ...
                                                        if (hdnrblbilltypeCredit == true && dsCompanyforLOA != null)
                                                        {
                                                            if (dsCompanyforLOA.Tables.Count == 2)
                                                            {
                                                                AssignLOAConfigDetails(dsCompanyforLOA.Tables[1].Copy(), 1, Convert.ToInt32(hdnPatientID), Convert.ToInt32(strCompanyID), Convert.ToInt32(strGradeID), Convert.ToInt32(drNewRow["specialiseId"]), PatientBillList);
                                                            }
                                                        }
                                                        if (Convert.ToInt32(hdnLOAfollowupLimit) > 0 || Convert.ToInt32(hdnLOAfollowupDays) > 0)
                                                        {
                                                            ...
                                                            {
                                                                StrOrdTyp = CheckLOAConsultationFollowupConfig(Convert.ToInt32(hdnPatientID), Convert.ToInt32(drNewRow["specialiseId"]), Convert.ToInt32(drNewRow["ProcedureId"]), Convert.ToInt32(drNewRow["SampleId"]), Convert.ToString(drNewRow["Sample"]), Convert.ToInt32(hdnLOAfollowupDays), Convert.ToInt32(hdnLOAfollowupLimit), PatientBillList).Split(Convert.ToChar("/"));
                                                            }
                                                            if (StrOrdTyp.Length == 1)
                                                            { return; }
                                                            ...
                                                            {
                                                                hdnProcedureID = drNewRow["ProcedureID"].ToString();
                                                                drNewRow["SampleId"] = StrOrdTyp[0].ToString();
                                                                drNewRow["Sample"] = StrOrdTyp[1].ToString();
                                                            }
                                                        }
                                                    }
                                                    if (hdnLOAfollowupLimit == "0")
                                                    {
                                                        #region Yiaco Followup: bhaskar
                                                        string strYiaco = ConfigurationSettings.AppSettings["Yaico"].ToString();
                                                        if (strYiaco.ToUpper() == "YES")
                                                        {
                                                        ...
                                                        {
                                                            strDummy = CheckConsultationFollowupConfig(Convert.ToInt32(hdnPatientID), Convert.ToInt32(drNewRow["SpecialiseId"].ToString()), Convert.ToInt32(drNewRow["ProcedureID"].ToString()), Convert.ToInt32(drNewRow["SampleId"].ToString()), drNewRow["Sample"].ToString(), PatientBillList);
                                                        }
                                                        #endregion Yiaco Followup
                                                        StrOrdTyp = strDummy.Split(Convert.ToChar("/"));
                                                        if (StrOrdTyp.Length == 1)
                                            ...
                                            #region Agreement price
                                            string stragrementprice1 = ConfigurationSettings.AppSettings["AgreementPrice"].ToString();
                                            if (stragrementprice1.ToUpper() == "NO")
                                            {
                                            }
                                            else
                                            {
                                                if (!string.IsNullOrEmpty(hdnTariffID))
                                                    intTariffid = Convert.ToInt32(hdnTariffID);
                                            }
                                            #endregion                                          
                                            if (IsAppliedProfessionalCharge == true)
                                            {
                                                dtItemPrices = GetPriceListWithProfCharge(Convert.ToInt32(ddlservices), Convert.ToInt32(drNewRow["ProcedureID"].ToString()), intTariffid, -1, Convert.ToInt32(drNewRow["SampleId"]), Convert.ToInt32(drNewRow["SpecialiseId"].ToString()), Convert.ToInt32(strDefaultUserId), strBillBedType, Convert.ToInt32(strDefWorkstationId), 0).Tables[0];
                                            }
                                            else
                                            {
                                                dtItemPrices = GetPriceList(Convert.ToInt32(ddlservices), Convert.ToInt32(drNewRow["ProcedureID"].ToString()), intTariffid, -1, Convert.ToInt32(drNewRow["SampleId"]), Convert.ToInt32(drNewRow["SpecialiseId"].ToString()), Convert.ToInt32(strDefaultUserId), strBillBedType, Convert.ToInt32(strDefWorkstationId), 0).Tables[0];
                                            }
                                    ...
                                    default:
                                        #region Agreement price
                                        string stragrementprice = ConfigurationSettings.AppSettings["AgreementPrice"].ToString();
                                        if (stragrementprice.ToUpper() == "NO")
                                        {
                                            dtItemPrices = GetPriceList(Convert.ToInt32(ddlservices), Convert.ToInt32(drNewRow["ProcedureID"].ToString()), intTariffid, -1, 0, 0, Convert.ToInt32(strDefaultUserId), strBillBedType, Convert.ToInt32(strDefWorkstationId), 0).Tables[0];
                                        }
                                        else
                                        {
                                            if (!string.IsNullOrEmpty(hdnTariffID))
                                            ...
                                            if (IsAppliedProfessionalCharge == true)
                                            {
                                                dtItemPrices = GetPriceListWithProfCharge(Convert.ToInt32(ddlservices), Convert.ToInt32(drNewRow["ProcedureID"].ToString()), intTariffid, -1, 0, 0, Convert.ToInt32(strDefaultUserId), strBillBedType, Convert.ToInt32(strDefWorkstationId), 0).Tables[0];
                                            }
                                            else
                                            {
                                                dtItemPrices = GetPriceList(Convert.ToInt32(ddlservices), Convert.ToInt32(drNewRow["ProcedureID"].ToString()), intTariffid, -1, 0, 0, Convert.ToInt32(strDefaultUserId), strBillBedType, Convert.ToInt32(strDefWorkstationId), 0).Tables[0];
                                ...
                                if (hdnDocID != string.Empty)
                                {
                                    DataSet dsFetchResults = null;
                                    dsFetchResults = FetchDoctorMarkUpPercent(Convert.ToInt32(hdnDocID), Convert.ToInt32(strDefaultUserId), Convert.ToInt32(strDefWorkstationId), 0);

                                    if (dsFetchResults != null && dsFetchResults.Tables[0] != null && dsFetchResults.Tables[0].Rows.Count > 0)
                                    {
                                        foreach (DataRow dr in dsFetchResults.Tables[0].Rows)
                                        {
                                            if (hdnDocID == dr["DoctorID"].ToString() && hdnDocSpecialiseId == dr["SpecialiseID"].ToString() && System.DateTime.Now > Convert.ToDateTime(dr["EffectiveDate"]))
                                            {
                                                MarkUp = Convert.ToDecimal(dr["MarkUpPercent"]);
                                            }
                                        }
                                    }
                                }

                                DataRow[] drPrice = dtItemPrices.Select();

                                if (dtItemPrices.Rows.Count > 0)
                                {
                                    if (System.Configuration.ConfigurationManager.AppSettings["OPBillBasePrice"] != null)
                                    {
                                        if (System.Configuration.ConfigurationManager.AppSettings["OPBillBasePrice"].ToLower() == "yes")
                                        {
                                            if (!string.IsNullOrEmpty(OrderTypeM))
                                            {
                                                if (OrderTypeM.ToString().ToUpper().Contains("CONSULTATION"))
                                                {
                                                    if (FixedConsultation != null && FixedConsultation != "")
                                                    {
                                                        BasePrice = Convert.ToDecimal(Convert.ToInt32(FixedConsultation.ToString()));
                                                        if (BasePrice < 1)
                                                        {
                                                            BasePrice = Convert.ToDecimal(drPrice[0]["BasePrice"] == DBNull.Value ? 0 : drPrice[0]["BasePrice"]);
                                                        }
                                    ...
                                    if (MarkUp > 0)
                                    {
                                        ...
                                        EligiblePrice = Convert.ToDecimal(drPrice[0]["EligiblePrice"] == DBNull.Value ? 0 : drPrice[0]["EligiblePrice"]);
                                        EligiblePrice = EligiblePrice * (1 + MarkUp / 100);
                                        BillablePrice = Convert.ToDecimal(drPrice[0]["BillablePrice"] == DBNull.Value ? 0 : drPrice[0]["BillablePrice"]);
                                        BillablePrice = BillablePrice * (1 + MarkUp / 100);
                                    }
                                    if ((decimal)BasePrice == -1)
                                    {
                                        drNewRow["BasePrice"] = DBNull.Value;
                                        drNewRow["Price"] = Convert.ToDecimal("0").ToString(hdnsCurrencyFormat);
                                    }
                                    else
                                    {
                                        if ((decimal)BasePrice == 0)
                                        {
                                            drNewRow["BasePrice"] = Convert.ToDecimal("0").ToString(hdnsCurrencyFormat);
                                            drNewRow["Price"] = Convert.ToDecimal("0").ToString(hdnsCurrencyFormat);
                                        }
                                        else
                                        {
                                            drNewRow["BasePrice"] = Convert.ToDecimal(BasePrice).ToString(hdnsCurrencyFormat);
                                            drNewRow["Price"] = Convert.ToDecimal(BasePrice).ToString(hdnsCurrencyFormat);
                                        }
                                    }
                                    if ((decimal)EligiblePrice == -1)
                                    {
                                        drNewRow["EligiblePrice"] = DBNull.Value;
                                    }
                                    else
                                    {
                                        drNewRow["EligiblePrice"] = Convert.ToDecimal(EligiblePrice).ToString(hdnsCurrencyFormat);
                                    }
                                    if ((decimal)BillablePrice == -1)
                                    {
                                        drNewRow["BillablePrice"] = DBNull.Value;
                                    }
                                    else
                                    {
                                        drNewRow["BillablePrice"] = Convert.ToDecimal(BillablePrice).ToString(hdnsCurrencyFormat);
                                    }
                                    if ((decimal)EligiblePrice == -1 && (decimal)BillablePrice == -1)
                                    {
                                        drNewRow["Price"] = Convert.ToDecimal(BasePrice).ToString(hdnsCurrencyFormat);
                                    }
                                    else if ((decimal)EligiblePrice <= (decimal)BillablePrice)
                                    {
                                        if ((decimal)EligiblePrice >= 0)
                                            drNewRow["Price"] = Convert.ToDecimal(EligiblePrice).ToString(hdnsCurrencyFormat);
                                    }
                                    if (drNewRow["isgroup"].ToString() != "")
                                    {

                                    }
                                }
                            }
                            #region TestProfileChecking
                            if (drNewRow["isgroup"].ToString() != "")
                            {
                                if (Convert.ToBoolean(drNewRow["isgroup"]) == true)
                                {
                                    int Priority = 0;
                                    if (hdnradordeTypeRoutine == true)
                                        Priority = 13;
                                    else if (hdnradordeTypeASAP)
                                        Priority = 1;
                                    else
                                        Priority = 15;
                                    dtTestSpecimen = new DataTable();
                                    if (drNewRow["ServiceTypeId"].ToString() == "6")
                                    {
                                        drNewRow["ProfileId"] = Convert.ToInt32(drNewRow["ProcedureId"]);
                                        dtTestSpecimen = GetTestProfileSpecimen(Convert.ToInt32(drNewRow["ProcedureId"]), Priority, 3, 0, 0, 0);
                                        int intPrevId = 0;
                                        if (TestProfile != null)
                                        {
                                            dtTestsProfile = (DataTable)TestProfile.Copy();
                                            dtTestsProfile.AcceptChanges();
                                            if (dtTestsProfile != null && dtTestsProfile.Rows.Count > 0)
                                            {
                                            }
                                        }
                                        ...
                                        foreach (DataRow drsample in dtTestSpecimen.Rows)
                                        {
                                            if (intPrevId != Convert.ToInt32(drsample["TestID"]))
                                            {
                                                ...
                                                drTestProfile["Name"] = drsample["Name"];
                                                drTestProfile["SpecimenId"] = drsample["SpecimenID"];
                                                drTestProfile["ProfileId"] = Convert.ToInt32(drNewRow["ProcedureId"]);
                                                drTestProfile["SpecialiseId"] = drsample["SpID"];
                                                drTestProfile["Contribution"] = (Convert.IsDBNull(drsample["UnitRate"]) ? 0 : drsample["UnitRate"]);
                                                if (drsample["SpecimenName"] is DBNull)
                                                {
                                                ...
                                                if (strValidatePrfileAndPHCItems.ToUpper() == "YES" && strValidatePrfileAndPHCItems != null)
                                                {
                                                    for (int i = 0; i < dtTestsProfile.Rows.Count; i++)
                                                    {
                                                        DataRow drtest = dtTestsProfile.NewRow();
                                                        ...
                                                        if (drtest["TestID"].ToString() == drTestProfile["TestId"].ToString() && drtest["SpecialiseId"].ToString() == drTestProfile["SpecialiseId"].ToString())
                                                        {
                                                            strname1 += dtTestsProfile.Rows[i]["Name"].ToString() + "<br />";
                                                            if (strname1.Length > 0)
                                                            {
                                                                //System.Web.UI.ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','The Following item(s) Already Exists In The package or Profile:" + "<br />" + strname1 + "','OK','Information');", true);
                                                                TestProfile = null;
                                                                return;
                                                            }
                                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            dtTestSpecimen = FetchPackageItems(Convert.ToInt32(drNewRow["ProcedureId"]), 2, 0, Convert.ToInt32(strDefWorkstationId), 0);
                                            if (TestProfile != null)
                                            {
                                                dtTestsProfile = (DataTable)TestProfile.Copy();
                                                if (dtTestsProfile != null && dtTestsProfile.Rows.Count > 0)
                                    ...
                                    TestProfile = dtTestsProfile.Copy();
                                    TestProfile.AcceptChanges();
                                    OPPackageOrders = dtOPPackageOrders.Copy();
                                    OPPackageOrders.AcceptChanges();
                                }
                            }
                            #endregion TestProfileChecking

                            drNewRow["Quantity"] = 1;
                            drNewRow["MQTY"] = 1;
                            drNewRow["SQTY"] = 0;
                            drNewRow["Amount"] = drNewRow["BasePrice"];
                            drNewRow["BedTypeId"] = -1;
                            drNewRow["BedTypeName"] = "OPD";
                            if (!string.IsNullOrEmpty(hdnScheduleID))
                                drNewRow["ScheduleId"] = Convert.ToInt16(hdnScheduleID);
                            DTTemNew.Rows.Add(drNewRow);
                            if (dtItemPrices == null)
                                dtItemPrices = (DataTable)invtTable.Copy();
                            dtItemPrices.AcceptChanges();

                            if (IsAppliedProfessionalCharge == true)
                            {
                                if (dtItemPrices.Rows.Count > 1)
                                {
                                    DataRow[] drProfChargRecord = dtItemPrices.Select("ServiceId<>" + ddlservices);
                                    ProfChargeServiceId = Convert.ToInt32(drProfChargRecord[0]["ServiceId"]);
                                    ProfChargeServiceName = drProfChargRecord[0]["ServiceName"].ToString();

                                    DataRow dr = DTTemNew.NewRow();
                                    dr.ItemArray = drNewRow.ItemArray;
                                    decimal ProfChBasePrice, ProfChEligiblePrice, ProfChBillablePrice;
                                    ProfChBasePrice = Convert.ToDecimal(drProfChargRecord[0]["BasePrice"] == DBNull.Value ? 0 : drProfChargRecord[0]["BasePrice"]);
                                    ProfChEligiblePrice = Convert.ToDecimal(drProfChargRecord[0]["EligiblePrice"] == DBNull.Value ? 0 : drProfChargRecord[0]["EligiblePrice"]);
                                    ProfChBillablePrice = Convert.ToDecimal(drProfChargRecord[0]["BillablePrice"] == DBNull.Value ? 0 : drProfChargRecord[0]["BillablePrice"]);

                                    if ((decimal)ProfChBasePrice == -1)
                                    {
                                        dr["BasePrice"] = DBNull.Value;
                                        dr["Price"] = Convert.ToDecimal("0").ToString(hdnsCurrencyFormat);
                                    }
                                    else
                                    {
                                        if ((decimal)ProfChBasePrice == 0)
                                        {
                                            dr["BasePrice"] = Convert.ToDecimal("0").ToString(hdnsCurrencyFormat);
                                            dr["Price"] = Convert.ToDecimal("0").ToString(hdnsCurrencyFormat);
                                        }
                                        else
                                        {
                                            dr["BasePrice"] = Convert.ToDecimal(ProfChBasePrice).ToString(hdnsCurrencyFormat);
                                            dr["Price"] = Convert.ToDecimal(ProfChBasePrice).ToString(hdnsCurrencyFormat);
                                        }
                                    }
                                    if ((decimal)ProfChEligiblePrice == -1)
                                    {
                                        dr["EligiblePrice"] = DBNull.Value;
                                    }
                                    else
                                    {
                                        dr["EligiblePrice"] = Convert.ToDecimal(ProfChEligiblePrice).ToString(hdnsCurrencyFormat);
                                    }

                                    if ((decimal)ProfChBillablePrice == -1)
                                    {
                                        dr["BillablePrice"] = DBNull.Value;
                                    }
                                    else
                                    {
                                        dr["BillablePrice"] = Convert.ToDecimal(ProfChBillablePrice).ToString(hdnsCurrencyFormat);//BillablePrice

                                    }

                                    if ((decimal)ProfChEligiblePrice == -1 && (decimal)ProfChBillablePrice == -1)
                                    {
                                        dr["Price"] = Convert.ToDecimal(ProfChBasePrice).ToString(hdnsCurrencyFormat);
                                    }
                                    else if ((decimal)ProfChEligiblePrice <= (decimal)ProfChBillablePrice)
                                    {
                                        if ((decimal)ProfChEligiblePrice >= 0)
                                            dr["Price"] = Convert.ToDecimal(ProfChEligiblePrice).ToString(hdnsCurrencyFormat);
                                    }

                                    dr["ServiceId"] = ProfChargeServiceId;
                                    dr["ServiceName"] = ProfChargeServiceName;
                                    dr["Amount"] = Convert.ToDecimal(ProfChBasePrice).ToString(hdnsCurrencyFormat);
                                    DTTemNew.Rows.Add(dr);
                                }
                            }
                            if (DTTemNew.Select("ServiceID=2").Length > 0 & ConfigurationManager.AppSettings["EMRAutoTests"] != null)
                            {
                                if (!string.IsNullOrEmpty(hdnTariffID))
                                    intTariffid = Convert.ToInt32(hdnTariffID);

                                string strEMRAutoTests = ConfigurationManager.AppSettings["EMRAutoTests"].ToString();
                                string[] strAutoTestID = strEMRAutoTests.Split(',');

                                foreach (string testid in strAutoTestID)
                                {
                                    dtItemPrices = GetPriceList(5, Convert.ToInt32(testid), intTariffid, -1, 0, 0, Convert.ToInt32(strDefaultUserId), strBillBedType, Convert.ToInt32(strDefWorkstationId), 0).Tables[0];
                                    DataRow dr = DTTemNew.NewRow();
                                    dr.ItemArray = drNewRow.ItemArray;
                                    ...
                                    {
                                        dr["BillablePrice"] = Convert.ToDecimal(ProfChBillablePrice).ToString(hdnsCurrencyFormat);
                                    }
                                    if ((decimal)ProfChEligiblePrice == -1 && (decimal)ProfChBillablePrice == -1)
                                    {
                                        dr["Price"] = Convert.ToDecimal(ProfChBasePrice).ToString(hdnsCurrencyFormat);
                    ...
                    if (DTTem == null)
                        DTView = DTTemNew.Clone();
                    else
                    {
                        DTView = (DataTable)DTTem.Copy();
                        DTView.AcceptChanges();
                    }

                    if (DTTemNew == null) { return; }
                    if (DTTemNew.Rows.Count <= 0) { return; }
                    string strduplicates = string.Empty;
                    for (int intCnt = 0; intCnt < DTTemNew.Rows.Count; intCnt++)
                    {
                        //Checking for duplicates. 
                        if (DTView.Select("SampleId = " + DTTemNew.Rows[intCnt]["SampleId"] + " and ProcedureID=" + DTTemNew.Rows[intCnt]["ProcedureID"] + Servicefilter + DTTemNew.Rows[intCnt]["ServiceId"]).Length <= 0)
                        {
                            DTView.ImportRow(DTTemNew.Rows[intCnt]);
                        }
                        else
                            //for manipal CR:167
                            if (strValidatePrfileAndPHCItems.ToUpper() == "YES" && strValidatePrfileAndPHCItems != null && strValidatePrfileAndPHCItems != string.Empty)
                        {
                            DataTable dtchkprofile = (DataTable)TestProfile.Copy();
                            if (dtchkprofile != null)
                            {
                                if (dtchkprofile.Rows.Count > 0)
                                {
                                    string strProfileName = "";
                                    string strProcedureName = "";
                                    for (int intprof = 0; intprof < dtchkprofile.Rows.Count; intprof++)
                                    {
                                        DataRow drchkprofile = dtchkprofile.NewRow();
                                        drchkprofile["TestId"] = dtchkprofile.Rows[intprof]["TestID"];
                                        drchkprofile["ProfileId"] = dtchkprofile.Rows[intprof]["ProfileId"];
                                        drchkprofile["ServiceID"] = dtchkprofile.Rows[intprof]["Serviceid"];
                                        drchkprofile["SpecialiseId"] = dtchkprofile.Rows[intprof]["SpecialiseId"];
                                        DTTemp = (DataTable)DTTem.Copy();
                                        DTTemp.AcceptChanges();
                                        if (DTView.Rows.Count > 0)
                                        {
                                            for (int ii = 0; ii < DTView.Rows.Count; ii++)
                                            {
                                                DataRow drDTTemNew = DTView.NewRow();
                                                drDTTemNew["ProcedureId"] = DTView.Rows[ii]["ProcedureId"];
                                                if (drchkprofile["TestId"].ToString() == drDTTemNew["ProcedureId"].ToString())
                                                {
                                                    string ProfileID = dtchkprofile.Select("TestId='" + drchkprofile["TestId"].ToString() + "'")[0]["ProfileId"].ToString();
                                                    strProfileName += DTView.Select("ProcedureId='" + ProfileID + "'")[0]["Procedure"].ToString() + "<br />";
                                                    strProcedureName += DTView.Rows[ii]["Procedure"].ToString() + "<br />";
                                                    DTView.Rows.RemoveAt(ii);
                                                }
                                                else if (drchkprofile["ServiceID"].ToString() == DTView.Rows[ii]["Serviceid"].ToString())
                                                {
                                                    if (drchkprofile["SpecialiseId"].ToString() == DTView.Rows[ii]["SpecialiseId"].ToString())
                                                    {
                                                        string ProfileID = dtchkprofile.Select("TestId='" + drchkprofile["TestId"].ToString() + "'")[0]["ProfileId"].ToString();
                                                        strProfileName += DTView.Select("ProcedureId='" + ProfileID + "'")[0]["Procedure"].ToString() + "<br />";
                                                        strProcedureName += DTView.Rows[ii]["SpecialiseName"].ToString() + "<br />";
                                                        DTView.Rows.RemoveAt(ii);
                                                        Validation = strProcedureName;
                                                        ValidationProfile = strProfileName;
                                                    }
                                    ...
                                    {
                                        //System.Web.UI.ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','The Following item(s) Already Exists In The" + " " + strProfileName + "Package:" + "<br />" + strProcedureName + "','OK','Information');", true);
                                        return;
                                    }
                                }
                            }
                        ...
                        {
                            if (strduplicates == string.Empty)
                            {
                                strduplicates = "<br /> ";
                            }
                            strduplicates += "Procedure: " + DTTemNew.Rows[intCnt]["Procedure"] + " with Sample: " + DTTemNew.Rows[intCnt]["Sample"] + "<br /> ";
                        }
                    }
                    if (strduplicates != string.Empty)
                    {
                        //System.Web.UI.ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "',' " + strduplicates + " Already Exists Increase the Qty to raise Multiple items','OK','Information');", true);
                    }
                    if (System.Configuration.ConfigurationManager.AppSettings["MiscDoctorEntry"] != null)
                    {
                        if (System.Configuration.ConfigurationManager.AppSettings["MiscDoctorEntry"].ToString().ToUpper() == "YES")
                        {
                            DataRow[] drMisc1 = DTView.Select("ServiceId = " + 6);
                            DataTable dtMisc = DTView.Clone();
                            foreach (DataRow row in drMisc1)
                            {
                                dtMisc.ImportRow(row);
                            }
                            MiscDoctorEntry = dtMisc.Copy();
                            MiscDoctorEntry.AcceptChanges();
                        }
                    }
                    ...
                    if (DTView.Rows.Count > 0)
                    {
                        ...
                        if (gdvSearchResultData.Rows.Count > 0)
                        {
                            foreach (DataRow grow in gdvSearchResultData.Rows)
                            {
                                DTView.Rows[i]["Quantity"] = grow["Quantity"].ToString();
                                string strPrice = grow["Price"].ToString();
                                if (Convert.ToDouble(strPrice) != 0.0)
                                {
                                    DTView.Rows[i]["Quantity"] = grow["Quantity"].ToString();
                                    DTView.Rows[i]["Price"] = grow["Price"].ToString();
                                }
                                ++i;
                            }
                        }
                        ...
                        if (DocCode != null && DocCode.ToString() != "")
                        {
                            if ((object)(ConfigurationManager.AppSettings["OPBillingDoctorCode"]) != null)
                                if (System.Configuration.ConfigurationManager.AppSettings["OPBillingDoctorCode"] == DocCode.ToString())
                                {
                                    DataRow[] dr_Consult = DTView.Select("procedureid=-2");
                                    if (dr_Consult.Length > 0)
                                    {
                                        foreach (DataRow dr in dr_Consult)
                                        {
                        ...
                        if (DTView != null)
                            query = from dtView in DTView.AsEnumerable()
                                    select dtView.Field<int>("ServiceId");
                        else
                            query = null;
                        if (!DTView.Columns.Contains("ProcId"))
                            DTView.Columns.Add("ProcId");
                        if (query != null && query.Contains(28) && query.Count<int>() == 1)
                        {
                            ddlservices = "28";
                            gdvSearchResultData = DTView.Copy();
                            gdvSearchResultData.AcceptChanges();
                            DTTem = DTView.Copy();
                            DTTem.AcceptChanges();
                        }
                        ...
                        gdvSearchResultData = DTView.Copy();
                        gdvSearchResultData.AcceptChanges();
                        GridContents = DTView.Copy();
                        GridContents.AcceptChanges();
                        searchData = null;
                        searchData = null;
                        DTTem = DTView.Copy();
                        DTTem.AcceptChanges();
            ...
            {
                HIS.TOOLS.Logger.ErrorLog.ErrorRoutine(ex, MODULE_NAME, "Error in imgItemprocedure_Click Event", "");
                //ScriptManager.RegisterStartupScript(Page, Page.GetType(), this.Title, "ShowMsgBox(' " + this.Title + " ', 'Unable to process the request' , 'OK, 'Error');", true);
            }
        ...
        private void SetSearchValuesToDataRow(int j, DataTable tblselectedData, DataRow drNewRow)
        {
            try
            {
                drNewRow["ProfileId"] = 0;
                drNewRow["ProcedureID"] = tblselectedData.Rows[j]["ProcedureID"].ToString();

                string itemid = tblselectedData.Rows[j]["ProcedureID"].ToString();
                if (hdnPrice == "Investigation")
                {
                    int intWorkstationid = Convert.ToInt32(strDefWorkstationId);
                    int intPriority;
                    ...
                    else if (hdnradordeTypeASAP == true)
                    {
                        intPriority = 1;
                        Priority = null;
                    }
                    ...
                    {
                        intPriority = 15;
                        Priority = intPriority.ToString();
                    }
                    int intTariffid = -1, intSpecialiseID = 0, intUserid = 0, intError = 0;
                    #region Agreement price : bhaskar on 03-08-2012
                    string stragrementprice = ConfigurationSettings.AppSettings["AgreementPrice"];
                    if (stragrementprice.ToUpper() == "NO")
                    {
                        if (hdnrblbilltypeCredit == true)
                            intTariffid = Convert.ToInt32(hdnTariffID);
                    ...
                    #endregion
                    string strBillBedType = "-1"; //need to take it from search textbox;
                    //Investigation
                    dtItemPrices = IsAppliedProfessionalCharge ? GetPriceListWithProfCharge(Convert.ToInt32(ddlservices), Convert.ToInt32(drNewRow["ProcedureID"].ToString()), intTariffid, -1, intPriority, intSpecialiseID, Convert.ToInt32(strDefaultUserId), strBillBedType, intWorkstationid, intError).Tables[0] : FetchPriceDetailsNew(Convert.ToInt32(ddlservices), Convert.ToInt32(itemid), intTariffid, -1, intPriority, intSpecialiseID, intUserid, strBillBedType, intWorkstationid, intError, 0).Tables[0];

                    drNewRow["Priority"] = intPriority;
                    DataRow[] drPrice = dtItemPrices.Select();
                ...
                drNewRow["Procedure"] = tblselectedData.Rows[j]["code"].ToString() + "-" + tblselectedData.Rows[j]["Name"].ToString();

                DocCode = tblselectedData.Rows[j]["Code"].ToString();
                drNewRow["ServiceId"] = tblselectedData.Rows[j]["ServiceID"].ToString();
                drNewRow["ServiceName"] = tblselectedData.Rows[j]["Service"].ToString();

                if (tblselectedData.Rows[j]["SpecialiseId"] != DBNull.Value)
                    drNewRow["SpecialiseId"] = tblselectedData.Rows[j]["SpecialiseId"].ToString();
                else
                    drNewRow["SpecialiseId"] = 0;
                drNewRow["SpecialiseName"] = tblselectedData.Rows[j]["Specialisation"].ToString();

                if (tblselectedData.Columns.Contains("SpecimenID"))
                {
                    if (tblselectedData.Rows[j]["SpecimenID"] != DBNull.Value)
                        drNewRow["SampleId"] = tblselectedData.Rows[j]["SpecimenID"].ToString();
                    else
                        drNewRow["SampleId"] = 0;
                }
                if (tblselectedData.Columns.Contains("specimenname"))
                {
                    drNewRow["Sample"] = tblselectedData.Rows[j]["specimenname"].ToString();
                }
                drNewRow["IsGroup"] = tblselectedData.Rows[j]["IsGroup"].ToString();
                if (tblselectedData.Rows[j]["PatientType"] != DBNull.Value)
                    drNewRow["PatientType"] = tblselectedData.Rows[j]["PatientType"].ToString();
                else
                    drNewRow["PatientType"] = 1;

                drNewRow["ServiceTypeID"] = tblselectedData.Rows[j]["ServiceTypeID"].ToString();
                if (tblselectedData.Rows[j]["HospDeptID"] != DBNull.Value)
                    drNewRow["DeptId"] = tblselectedData.Rows[j]["HospDeptID"].ToString();
                else
                    drNewRow["DeptId"] = 0;
                drNewRow["DeptName"] = tblselectedData.Rows[j]["DepartmentName"].ToString();
                if (tblselectedData.Columns.Contains("OrderType"))
                {
                    if (tblselectedData.Rows[j]["OrderType"] != DBNull.Value)
                    {
                        drNewRow["Sample"] = tblselectedData.Rows[j]["OrderType"].ToString();
                        hdnordertypeName = tblselectedData.Rows[j]["OrderType"].ToString();
                    }
                }
                if (tblselectedData.Columns.Contains("OrderTypeID"))
                {
                    if (tblselectedData.Rows[j]["OrderTypeID"] != DBNull.Value)
                    {
                        drNewRow["SampleId"] = tblselectedData.Rows[j]["OrderTypeID"].ToString();
                        hdnordertypeID = tblselectedData.Rows[j]["OrderTypeID"].ToString();
                    }
                }
                ...
                {
                    if (tblselectedData.Rows[j]["OrderTypeID"] != DBNull.Value)
                        hdnintBillDocEmpId = tblselectedData.Rows[j]["OrderTypeID"].ToString();
                }
                invtTable = dtItemPrices;
            }
            finally
            {
        ...
        public DataSet FetchPriceDetailsNew(int intServiceid, int intItemID, int intTariffid, int intBillType, int intPriority, int intSpecialiseid, int intUserID, string strBedTypeid, int intWorkstationId, int intError, int type)
        {
            try
            {
                objFOClient = new FrontOfficeServiceContractClient();
                return objFOClient.FetchPriceDetailsNew(intServiceid, intItemID, intTariffid, intBillType, intPriority, intSpecialiseid, strBedTypeid, intUserID, intWorkstationId, intError, type);
            }
        ...
        public DataSet FetchDoctorMarkUpPercent(int DoctorID, int intUserID, int intWorkStationID, int intError)
        {
            DataSet dsFetchResults = null;
            AdminClient objAdminClient = new AdminClient();
            try
            {
                dsFetchResults = objAdminClient.FetchDoctorMarkUpPercent(DoctorID, intUserID, intWorkStationID, intError);
            }
            finally
            {
                objAdminClient.Close();
            }
            return dsFetchResults;
        }

        private void AssignPayments(PatientBillDepositList PatientBillList)
        {
            decimal decCurrencyRate = 1;
            if (PatientBillList.ModeOfPayment.ToString() != "6")
            {
            ...
            drMulti["PMID"] = PatientBillList.ModeOfPayment.ToString();
            if (PatientBillList.ModeOfPayment.ToString() != "6")
            {
                drMulti["CID"] = PatientBillList.CardType.ToString();
                drMulti["BID"] = PatientBillList.BankName.ToString();
            }
            drMulti["EDATE"] = Edate.ToString("dd-MMM-yyyy");
            drMulti["EID"] = DBNull.Value;
            drMulti["ET"] = "DI";
            drMulti["AMT"] = PatientBillList.Amount.ToString();
            drMulti["TNO"] = " ";
            drMulti["CNO"] = PatientBillList.CreditCardNO.ToString();
            drMulti["TDATE"] = Edate.ToString("dd-MMM-yyyy");
            drMulti["VTO"] = Convert.ToDateTime(PatientBillList.CardValidity.ToString()).ToString("dd-MMM-yyyy");
            ...
            drMulti["CUID"] = 5;
            drMulti["CURS"] = 1;
            drMulti["CAMT"] = PatientBillList.Amount.ToString();
        ...
        {
            DataHelper objDataHelper = new DataHelper(DEFAULTWORKSTATION, (int)Database.Master, intHospitalsId);
            DataSet ds = null;
            try
            {
                List<IDbDataParameter> objIDbDataParameters = new List<IDbDataParameter>();
        ...
        public PatientBillInfoDepositList SavePatientDepositList(PatientBillDepositList PatientBillList)
        {
            DataHelper objDataHelper = new DataHelper(DEFAULTWORKSTATION, (int)Database.Master, PatientBillList.HospitalId);
            PatientBillInfoDepositList objPatientList = new PatientBillInfoDepositList();
            strDefaultHospitalId = PatientBillList.HospitalId.ToString();
            try
            {
                ...
                decimal Amount; int BlockStatus = 1;//     Blocked for OP;
                Amount = Convert.ToDecimal(PatientBillList.Amount);
                string TransationNo = " "; int customerID = -1; int IsXmL = 1;
                if (PatientBillList.ModeOfPayment == 2 || PatientBillList.ModeOfPayment == 3)
                {
                    CardNo = PatientBillList.CreditCardNO;
                    BankId = Convert.ToInt32(PatientBillList.BankName);
                    CardId = Convert.ToInt32(PatientBillList.CardType);
                    ValidDate = Convert.ToDateTime(PatientBillList.CardValidity).ToString("dd-MMM-yyyy");
                }
                else
                {

                }
                AssignPayments(PatientBillList);
                string strXMLPaymentDetails = Utilities.ConvertDTToXML("PAYMENTS", "ITEM", dtMultiPayment);
                List<IDbDataParameter> objIDbDataParameters = new List<IDbDataParameter>();
                objIDbDataParameters.Add(CreateParam1(objDataHelper, "@EntryID", null, 100, DbType.Int32, ParameterDirection.Output));
                objIDbDataParameters.Add(CreateParam1(objDataHelper, "@EntryNo", null, 100, DbType.String, ParameterDirection.Output));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@EntryDate", EntryDate.ToString() == "" ? null : EntryDate, DbType.DateTime, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@PatientID", 0, DbType.String, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@PaymentModeID", PatientBillList.ModeOfPayment, DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@Amount", Amount, DbType.Decimal, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@TransationNo", TransationNo, DbType.String, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@CardNo", CardNo, DbType.String, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@CardID", CardId, DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@BankID", BankId, DbType.String, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@CustomerID", customerID, DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@IssueDate", IssueDate.ToString() == "" ? null : IssueDate, DbType.DateTime, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@ValidTill", ValidDate.ToString() == "" ? null : ValidDate, DbType.DateTime, ParameterDirection.Input));

                objIDbDataParameters.Add(CreateParam(objDataHelper, "@Contact", null, DbType.String, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@Remarks", null, DbType.String, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@BlockID", BlockStatus, DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@BlockedFor", BlockStatus, DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@UserID", Convert.ToInt32(strDefaultUserId), DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@WorkStationID", Convert.ToInt32(strDefWorkstationId), DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@Error", 0, DbType.Int32, ParameterDirection.Output));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@ISXML", IsXmL, DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@payments", strXMLPaymentDetails, DbType.String, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@currencyid", 5, DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@CurrencyRate", 1, DbType.Decimal, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@CurrencyAmount", Amount, DbType.Decimal, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@NottoPost", 0, DbType.Boolean, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@ScheduleId", Convert.ToInt32(PatientBillList.ScheduleID), DbType.Int32, ParameterDirection.Input));
                objIDbDataParameters.Add(CreateParam(objDataHelper, "@UHID", PatientBillList.RegCode, DbType.String, ParameterDirection.Input));
                int intRes = objDataHelper.RunSP("Pr_SaveAccountDeposits_MAPI", objIDbDataParameters.ToArray());
                if (int.Parse(objIDbDataParameters[0].Value.ToString()) > 0)
                {
                    PatientBillInfoDepositListN obj = new PatientBillInfoDepositListN();
                    string sDepositNo = Convert.ToString(objIDbDataParameters[1].Value.ToString());
                    obj.DepositNO = sDepositNo;
                    objPatientList.BillDepositSummary.Add(obj);
                    objPatientList.Code = ProcessStatus.Success;
                    objPatientList.Status = ProcessStatus.Success.ToString();
                    objPatientList.Message = "Deposit Number :" + sDepositNo + " Generated Successfully";
                }
                else
                {
                    objPatientList.Code = ProcessStatus.Fail;
                    objPatientList.Status = ProcessStatus.Fail.ToString();
                    objPatientList.Message = "Couldnot Generate the Deposit";
                }
                return objPatientList;
            ...
            catch (Exception ex)
            {
                ...
                objPatientList.Status = ProcessStatus.Fail.ToString();
                objPatientList.Message = ex.Message;
                HIS.TOOLS.Logger.ErrorLog.ErrorRoutine(ex, MODULE_NAME, "Error in SavePatientDepositList", "");
            }
            finally
            {
                objDataHelper = null;
